...
Cada archivo que ingresa a la plataforma se clasifica por tipo antes de que ocurra cualquier procesamiento. Los datos estructurados — hojas de cálculo, entradas de libro mayor, exportaciones de bases de datos, documentos firmados — se dirigen a motores de análisis deterministas que producen resultados verificables y reproducibles. El texto humano no estructurado — correspondencia, notas de reuniones, informes narrativos — se ingiere por la misma ruta determinista y sin IA: ninguna clasificación de contenido, ninguna extracción de entidades, ni llamada alguna a un modelo de lenguaje se ejecuta en línea durante la ingesta. Cualquier razonamiento de IA aplicado a ese texto ocurre únicamente aguas abajo, leyendo registros ya escritos desde el libro mayor — nunca durante la propia escritura. Nada cruza el límite sin clasificar, y nada en el propio límite se deriva de la IA. Esto evita que la plataforma aplique razonamiento de IA a registros que deberían recibir un tratamiento determinista y auditable.
...
**ADR-16 — Los comandos entre pares viajan como difusión sin intermediario, filtrada por destinatario.**
Los comandos enviados a los nodos pares en la red de malla de la plataforma viajan por difusión UDP a la dirección de difusión de la subred de malla — cada nodo en la malla cifrada con WireGuard recibe cada comando, y la carga útil incluye un campo `target` que cada nodo usa para decidir si el comando le corresponde. No existe un intermediario central que reenvíe comandos a nodos individuales; la malla no tiene un único punto de fallo para la entrega. Esto intercambia privacidad de entrega a nivel de red (cualquier nodo puede observar el tráfico dirigido a otro) por una topología de malla más simple, sin intermediario que mantener disponible.
