...
**Corrección (2026-08-03, paridad con la versión en inglés, verificada contra `origin/main` canónico):** la elaborada arquitectura de intermediación de capacidades/federación/aplicación de ausencia de estado descrita más abajo no existe. El verdadero `os-orchestration/src/lib.rs` es un scaffold de 2 líneas (`"SYSTEM EVENT: os-orchestration scaffold verified."`) — sin intermediarios de capacidades, sin modelo de federación, sin lógica de Anillos de nivel comercial. Además, la sección del Modelo Comercial de este artículo atribuía incorrectamente a `os-orchestration` un sistema de ingresos y medición que en realidad pertenece a un crate distinto y real: `app-orchestration-slm` (`crates/orchestration-slm/src/metering.rs`, confirmado en `origin/main`). Todo el artículo se reformula a continuación con lenguaje previsto/planeado, y la sección Comercial se corrige para atribuir el modelo de Nivel B a `app-orchestration-slm`.

La plataforma PointSav está diseñada alrededor de un límite arquitectónico deliberado: una capa de agregación pensada para coordinar el trabajo entre los [[totebox-archive|Archivos Totebox]] sin retener datos de clientes, sin almacenar claves y sin escribir nada en ningún [[worm-ledger-architecture|libro de registro WORM]]. Esta capa está planeada como `os-orchestration` — hoy un crate de scaffold registrado pero sin implementar, no un sistema en funcionamiento (véase la corrección anterior).
...
**Esta sección atribuía anteriormente a `os-orchestration` un sistema de ingresos y medición en funcionamiento. No lo tiene — `os-orchestration` es un scaffold sin implementar (véase la corrección anterior). El verdadero modelo de ingresos y medición por uso de Nivel B pertenece a un crate distinto y real: `app-orchestration-slm`.** `app-orchestration-slm` es un "chasis intermediario Yo-Yo" en funcionamiento (confirmado en `origin/main` canónico, puerto `:9180`) que conecta las instancias Doorman de `service-slm` de los Archivos Totebox con una flota de GPU Yo-Yo compartida. Implementa una medición real de costos por inquilino (`crates/orchestration-slm/src/metering.rs`): el costo de cada solicitud de inferencia se calcula a partir del tiempo de inferencia medido y una tarifa horaria en USD configurada, y se registra en un libro de contabilidad por inquilino contra el cual se factura a un cliente de Nivel B. El Nivel A — el nivel de inferencia local y gratuito — no se ve afectado por esto y no requiere medición. Si `os-orchestration` llega a construirse como capa de agregación/federación, podría en principio enrutar solicitudes hacia el servicio de Nivel B de `app-orchestration-slm`, pero la lógica de medición, facturación y acumulación de ingresos reside en `app-orchestration-slm`, no en `os-orchestration`.

## El Intermediario de GPU Yo-Yo

`app-orchestration-slm` es el chasis [[yoyo-compute-substrate|intermediario de GPU Yo-Yo]] real y en funcionamiento (véase la corrección del Modelo Comercial anterior), que implementa hoy la asignación de GPU bajo demanda y la medición por inquilino. Cuando el Doorman de `service-slm` de un Archivo Totebox envía una solicitud de inferencia que supera la capacidad local de Nivel A — ya sea porque el modelo es demasiado grande para el hardware local o porque la carga concurrente ha agotado el cómputo disponible — la solicitud se enruta al chasis de `app-orchestration-slm`.
