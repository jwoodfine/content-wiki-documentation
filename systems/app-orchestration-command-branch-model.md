...
**Correction (2026-08-02):** `app-orchestration-command` is not a PointSav software product. No crate or directory of that name exists anywhere in the monorepo (confirmed via directory listing and a corpus-wide search); the real `app-orchestration-*` family is `app-orchestration-{bim,exchange,gis,graph,market,slm}`. This article's own `research_trail` frontmatter cites `AGENT.md` and `pairings.yaml` as sources — both are the Foundry *workspace's own internal* multi-archive AI-development coordination tooling (the "Command Session" that manages this workspace's ~25 software archives), not a shipping PointSav OS-family coordinator. The isolation and publication mechanics described below are broadly accurate as a description of that internal Foundry tooling (`bin/commit-as-next.sh`, cluster branches, `bin/promote.sh`), but the article presents them, in unhedged present tense, as a customer-facing product running on a host called `os-orchestration` — which is also not built (the real crate, `os-interface`, is a 4-file scaffold; the rename to `os-orchestration` is planned, not executed). **Flagged, not resolved** — this may be a legitimate case for a *separate* workspace-internal-tooling TOPIC (which would need Foundry-vocabulary stripping per this repo's own editorial discipline), not a rewrite of a PointSav product article that shouldn't exist under this name.
