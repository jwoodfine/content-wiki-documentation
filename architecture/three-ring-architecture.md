...
[[service-slm|`service-slm`]] is the single Ring 3 service. It implements the [[doorman-protocol|Doorman]] pattern: every request enters through one boundary, which routes among the three compute tiers and logs every call to the per-tenant audit ledger. No API key lives outside the Doorman boundary.
