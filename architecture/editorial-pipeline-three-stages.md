...
gracefully if that service is unreachable. Stage 3 routes through the [[doorman-protocol|Doorman]] to one of three
...
`draft-created` → `draft-refined` → `creative-edited` — that feeds the [[apprenticeship-substrate|apprenticeship corpus]]
