...
The product strategy follows an open-core pattern. The core operating systems ([[totebox-os|`os-totebox`]], [[console-os|`os-console`]], [[os-workplace|`os-workplace`]]) are intended for release under Apache 2.0 with a planned Sovereign Addendum ensuring that running instances remain freely transferable by the operator regardless of where they are hosted. The fleet-aggregator product ([[os-orchestration|`os-orchestration`]]) is intended to remain proprietary as the commercial revenue driver. (Correction, 2026-08-02, verified against canonical `origin/main`: the Apache-2.0 claim repeats the same error found on [[leapfrog-2030-architecture]] — real licenses are AGPL/AGPL/FSL, not Apache 2.0, for these three. The `os-orchestration` "intended to remain proprietary" claim, however, is independently confirmed correct — the real ratified `LicenseTier::Proprietary` in the marketplace code explicitly names `os-orchestration`/`app-orchestration-*` as the company's stated commercial moat. Flagged, not resolved for the Apache-2.0 clause only.)
