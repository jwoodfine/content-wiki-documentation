...
**Why it matters:** the record is owned the way the building itself is owned — permanently, transferably, and without paying a vendor for continued access to it.

## The vault layout

The canonical vault for a property archive is structured as follows:
...
**Why it matters:** sensor and lease data are usually the first casualty of a vendor migration — a flat sidecar file survives one because nothing about it depends on the tool that wrote it.

## The hash-addressed object store and its token structure

The `vault/objects/` directory implements a hash-addressed object store. Each object is a JSON file whose filename is the SHA-256 hash of its content — this is the archive's unit of stored data, and each one combines three things at once, written together as a single atomic record: an immutable binary payload that is content-addressable and write-once; a metadata skeleton carrying schema-validated fields describing the content's type, provenance, and ownership; and a graph connection linking the object to its position in the taxonomic hierarchy maintained by the knowledge-graph service. No one of the three exists in isolation within the archive — a payload without its metadata skeleton or its graph position is not a resolvable object.

`vault/refs/` holds named pointers — branches, tags, and HEAD — that resolve to specific object hashes.
...
A physical property asset transfers with a title deed. The vault is intended to be bundled with that transfer: the same git repository that holds the IFC archive, the lease register references, and the operational history accompanies the property when ownership changes hands.
...
- [[flat-file-bim-leapfrog]] — the five architectural constraints this vault design is one expression of
- [[building-design-system]] — the coordination layer that organizes the object categories this vault stores
- [[bim-object-specification]] — the BIM Object schema each element in this vault carries
